...
### Why does the palette × hull_style cross-axis property test exist?

`test_property_palette_x_hull_style_seed_grid_*` in `tests/test_properties.py` pins (palette × `HullStyle` × seed) over a 27-node grid (3 × 3 × 3) to catch interaction-only regressions where palette role coverage and the hull silhouette individually validate but their *combination* silently no-ops — e.g. a palette stubbing the `hull_dark` role combined with a `dagger` hull whose narrow stern has no surface cells the texture pass can recolor, so the silhouette renders monochrome where the palette would otherwise emphasize a dark accent stripe. Slips past both single-axis siblings (`test_property_palette_seed_grid_*` and `test_property_hull_style_seed_grid_*`); slices the palette list dynamically (first / middle / last alphabetically of `_PALETTE_NAMES`) so a new YAML in `palettes/` automatically tracks the slice. See the [generic cross-axis FAQ entry](#why-do-cross-axis-property-tests-exist-alongside-single-axis-ones) above for the broader pattern.

### When should I use `--config-dump` vs `--from-manifest`?

Reach for `--config-dump` when you want to *capture the effective config of a CLI invocation as JSON* for inspection / sharing / scripting — it emits `{"effective_config":{...}}` to stdout reflecting the post-preset-resolution / post-`--palette random` resolution values that `generate()` would have received and exits 0 without writing a `.litematic`. Reach for `--from-manifest FILE` when you want a *byte-identical replay of a previous ship* — it reads `seed` / `palette` / dims from a `<name>.json` sidecar emitted by `--export-manifest` on the original run and rebuilds the same `.litematic` (mutex with `--seed` / `--seeds` / `--seed-phrase` / `--repeat` / `--fleet-count`). The two are complementary, not interchangeable: `--config-dump` is for "what would I run", `--from-manifest` is for "rebuild that exact ship". See [configuration.md](configuration.md#capturing-the-effective-config-via---config-dump) for the round-trip example.

### What is the `## Biome packs` section in `docs/palettes.md` for and how do I discover its pack IDs?

The `## Biome packs` H2 at the bottom of `docs/palettes.md` is a `Pack ID | Date | Palettes` table cataloging the dated 2-palette **biome packs** — each pack lands together under one `feat-palettes-biome-pack-<date>[suffix]` unit (1 commit / 1 CHANGELOG bullet / both palettes pass `palette_lint.py --strict` together) so reviewers can land a fresh role-coverage / hue family in isolation without disturbing the existing catalog. To discover a pack's originating commit and the design notes that landed with it, copy the `Pack ID` from the table and run `git log --oneline | grep <pack-id>` (or look it up in the matching `## [Unreleased]` bullet in [CHANGELOG.md](CHANGELOG.md)); to enumerate every palette a pack ships, read the third column of the same row.

### How does the web UI palette / preset picker work?

Two distinct picker surfaces, both real and independent. The **palette dropdown** is a server-side `<select name="palette" id="palette">` inside the SEED & PALETTE collapsible — options come from `GET /api/palettes` plus a `random (seeded)` sentinel; a `Random` button (`#random-palette-btn`) cycles in place and `#palette-swatches` paints role-color preview chips. The **preset picker** has two surfaces: (a) a server-side `<select name="preset" id="preset">` at the top of the sidebar populated from `presets.list_presets()` (named archetypes like `corvette` — server merges them under your form values via `_merge_preset_into_source`), and (b) a client-side **Presets drawer** opened from the topbar star button (`#btn-presets`, shortcut `P`) that saves/loads form snapshots to `localStorage["shipforge.presets.v1"]` (your saved configs, separate from server archetypes). They do not share state. See [web_ui.md](web_ui.md#picking-a-palette-and-preset) for the full walkthrough.
