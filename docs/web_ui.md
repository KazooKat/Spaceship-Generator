...
| `GET` | `/api/hull-styles` | — | Narrower sibling of `/api/styles` — only the `HullStyle` enum values. | `{hull_styles:[...]}` |
| `GET` | `/api/engine-styles` | — | Narrower sibling of `/api/styles` — only the `EngineStyle` enum values. | `{engine_styles:[...]}` |
| `GET` | `/api/wing-styles` | — | Narrower sibling of `/api/styles` — only the `WingStyle` enum values. | `{wing_styles:[...]}` |
| `GET` | `/api/roles` | — | List the `Role` enum (name + integer value) used in the shape_grid int8 array. Mirrors CLI `--list-roles-json`. | `{roles:[{name:"<NAME>",value:<int>},...]}` |
| `GET` | `/api/version` | — | Narrower JSON sibling of `/api/health` / `/api/meta` exposing only the installed package version string. Useful for about-box dialogs and deploy probes that don't need the rest of the health/meta payload. | `{version:"<X.Y.Z>"}` |
