...
dial: no engine, wing, or cockpit effects.

.. note:: **Legacy stampers.** :func:`apply_hull_style`,
   :func:`apply_hull_blend`, and their helpers (``hull_profile_fn``,
   ``hull_style_rx_ry``, ``hull_blend_weight``, ``blended_hull_radii``)
   are standalone preview utilities and are **not called by**
   ``generate_shape`` — the live pipeline shapes hulls via
   :mod:`spaceship_generator.shape.blueprint` massing configs. Editing
   these functions will not change generated ships.
"""
...
# (The old StructureStyle taper-profile functions and hull rx/ry scaling
# that drove the pre-v2 ellipsoid hull were removed when the blueprint
# massing stage took over — per-StructureStyle hull shaping now lives in
# spaceship_generator.shape.blueprint._STRUCTURE_MASSING.)
